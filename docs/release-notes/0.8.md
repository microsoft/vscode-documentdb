...
<!-- IMAGE PROMPT: VS Code terminal panel showing an Interactive Shell session. The terminal has a colored prompt "myDatabase>" with syntax-highlighted input: "db.users.find({ age: { $gt: 25 } })" where keywords are cyan, strings are green, numbers are yellow, and $-operators are yellow. Below the command, formatted JSON output shows 3 user documents. At the bottom, a line with two clickable links reads "↗ Collection View [myDatabase.users]  ↗ Query Playground [myDatabase.users]". A dim ghost text suggestion "find()" appears after a partially typed "db.orders." command at the new prompt. Use VS Code dark theme with the integrated terminal. An oversized text cursor (blinking bar) is visible at the prompt. Style: clean product screenshot illustration, DocumentDB blue accent (#0078D4), terminal with ANSI colors on dark background. Resolution: 1600x900. -->
