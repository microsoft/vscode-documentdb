...
This patch release restores DocumentDB Local demo-data loading and delivers dependency updates for improved security and reliability.

### What's Changed in v0.10.2

#### 💠 **Reliable DocumentDB Local Demo Data** ([#913](https://github.com/microsoft/vscode-documentdb/pull/913))

DocumentDB Local Quick Start now loads its sample data when using DocumentDB 0.116 and later. The setup detects the image's supported password interface while preserving compatibility with earlier images, so a new local instance is ready to explore with the included demo collections.
...
This release updates URI parsing, selector parsing, browser query, archive handling, IP address, and connection proxy dependencies. These updates address reported vulnerabilities and improve the extension's underlying reliability.

### Changelog

See the full changelog entry for this release:
➡️ [CHANGELOG.md#0102](https://github.com/microsoft/vscode-documentdb/blob/main/CHANGELOG.md#0102)
