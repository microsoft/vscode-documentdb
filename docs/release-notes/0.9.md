...
<!-- IMAGE PLACEHOLDER: 0.9.0_kubernetes_discovery.png
     Suggested screenshot: The full Discovery tree with the "Kubernetes Clusters" root expanded.
     Show at least two kubeconfig sources (e.g. "Default kubeconfig" and a custom file source),
     one context expanded to reveal namespaces (or clusters in list view), and two or three
     discovered DocumentDB target nodes — one with no connectivity word (LoadBalancer) and one
     with "port-forward" (ClusterIP) — so that the reachability labels are visible at a glance.
     Width: 800 px. -->
<p align="center"><img src="./images/0.9.0_kubernetes_discovery.png" alt="Kubernetes Service Discovery tree showing two kubeconfig sources, a context expanded into namespaces, and discovered DocumentDB targets with connectivity indicators" width="800" style="max-width:100%;height:auto;"></p>
...
- **Custom kubeconfig file**: pick any file from disk.
- **Pasted kubeconfig YAML**: paste YAML directly; it is kept in VS Code Secret Storage.
- **Drag-and-drop**: drop a kubeconfig file onto the Discovery view to register it instantly.

<!-- IMAGE PLACEHOLDER: 0.9.0_kubernetes_add_source.png
     Suggested screenshot: The "Add Kubeconfig…" quick pick open, showing the three options:
     "Default kubeconfig", "Add custom kubeconfig file…", and "Paste kubeconfig YAML from clipboard".
     Capture it while the quick pick is focused so the options list is visible.
     Width: 600 px. -->
<p align="center"><img src="./images/0.9.0_kubernetes_add_source.png" alt="Add Kubeconfig quick pick showing the three source options: Default kubeconfig, Add custom kubeconfig file, and Paste kubeconfig YAML from clipboard" width="600" style="max-width:100%;height:auto;"></p>
...
<!-- IMAGE PLACEHOLDER: 0.9.0_query_insights_streaming.png
     Suggested screenshot: The Query Insights panel mid-stream — ideally captured a few seconds
     after clicking "Get AI Performance Insights". The summary text should be partially visible
     (a few lines already rendered), one complete recommendation card should be showing, and
     a second card should be in its "loading skeleton" or partially revealed state, to illustrate
     the progressive reveal. The spinner or streaming indicator should still be active.
     A before/after side-by-side or a GIF/video would work even better if the docs site supports it.
     Width: 800 px. -->
...
Previously, the extension buffered the entire LLM response (which took 10–20 s to generate) before parsing and rendering it all at once. Users had no signal that anything was happening past the first second. The new experience connects the model's natural streaming output directly to the UI:
