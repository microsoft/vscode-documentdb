...
**Follow-up available, not built:** `GET /api/atlas/v2/groups/{groupId}/databaseUsers` (role
requirement **Project Read Only**, the same level the cluster listing already needs) returns every
database user in a project. Users are project-scoped, so narrowing to one cluster means filtering
each user's `scopes[]` for `{ name: <clusterName>, type: 'CLUSTER' }`; an empty `scopes` array
applies to every cluster. Passwords are never returned, so this can prefill the username prompt in
the connect wizard and detect the "only `$external` X.509/OIDC users exist" case, but it cannot skip
the password step. Not implemented here to keep this PR scoped.
