...
- **Empty `admin` filtering vs. `nameOnly` (resolved: reverted `nameOnly`).** The tree hides the `admin` database only when it is empty, so an `admin` database containing user-managed content (users, roles, custom collections) stays visible. That conditional hide depends on the `empty` field from `listDatabases`, which `nameOnly: true` omits (along with `sizeOnDisk`). We reverted the `nameOnly` optimization so `listDatabases()` returns the full listing and the empty-`admin` filter works again. The database-list payload is small, so the full listing is an acceptable cost for preserving the hide-when-empty behavior.
