...
- **Subscription lifecycle hardening.** `WebviewController` now tracks both the `AbortController` and the live `AsyncIterator` per subscription. `handleSubscriptionStopMessage` and `dispose()` now abort _and_ call `iterator.return()`, propagating cleanup through the procedure's async generator into any inner `for await`. `_activeSubscriptions.set` is deferred until after the iterator is in hand so early failures do not leave stale map entries. `_onDisposed` emitter registered in `_disposables` so it is torn down on panel dispose.
