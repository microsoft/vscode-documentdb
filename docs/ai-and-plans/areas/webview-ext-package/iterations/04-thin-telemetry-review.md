...
## Resolution status (post-review, verified 2026-07-15)

The author addressed every finding on the PR. Each fix was verified against the current working tree (branch `dev/tnaum/webview-ext-thin-telemetry`, HEAD `489de6bb`); the package host test suite passes (39/39). The two Copilot inline threads are both **resolved** on GitHub.

| ID    | Status               | Fix commit(s)                      | One-line resolution                                                                                                                                                          |
| ----- | -------------------- | ---------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| F1    | Fixed                | `0f3ed21e`                         | Version bumped `0.9.2` → `0.10.0`, moving the breaking release outside the `^0.9.1` caret range                                                                              |
| F2    | Fixed                | `76fbf1d9`                         | `onStart` now fires at the dispatch layer via `logProcedureStart`, paired one-to-one with `onEnd`                                                                            |
| F3/C2 | Fixed                | `efe086ee`                         | Model inverted: `actionContext` removed from base `RouterContext`, re-modeled as `WithTelemetry<T> = T & RpcEnrichment`; `getInfo` is now instrumented so its cast is honest |
| F4    | Fixed (pivoted)      | `34dc09ee`, `5f07c5f5`, `4c7cf5c9` | Skill guides updated to the new surface — **but keep `WithTelemetry`**, not the removal the original F4 recommended (see note)                                               |
| F5    | Fixed                | `797f007b`                         | Stale `trpcToTelemetry` comments in `queryInsightsEventsRouter.ts` refreshed to the middleware/runner model                                                                  |
| C1    | Resolved (won't-fix) | —                                  | Disproven via the tRPC v11 shallow-merge; thread resolved with the upstream reference. No regression test was added (author offered one, "say the word")                     |

### Fix details and reasons

- **F1 — `0f3ed21e`.** `package.json` `0.9.2` → `0.10.0`; `MIGRATION.md` heading and _After_ label re-pointed to `0.9.1 → 0.10.0`; `package-lock.json` synced. _Reason:_ a patch (`0.9.2`) satisfies the common `^0.9.1` range, so consumers would receive source-breaking changes on a clean install. The minor bump makes the upgrade opt-in.
- **F2 — `76fbf1d9`.** Added `logProcedureStart` in `attachTrpc.ts`; both `handleDefaultMessage` and `handleSubscriptionMessage` call it once before the handler's `try`, so an early setup failure still yields a start plus a matching `onEnd { ok: false }`. Transport tests added for ordering, exactly-once delivery, early subscription-setup failure, and an `onStart`-only logger; `ADVANCED.md` documents the pairing guarantee. _Reason:_ the dispatch path previously fired only `onEnd`, so an `onStart`-only logger passed through the documented panel option received nothing.
- **F3/C2 — `efe086ee`.** `actionContext` is no longer a field on base `RouterContext`; it is re-modeled as an additive enrichment `WithTelemetry<T> = T & RpcEnrichment` in `src/webviews/_integration/trpc.ts`. Instrumented procedures narrow to `WithTelemetry<RouterContext>`; plain `publicProcedure` procedures narrow to bare `RouterContext`, so reading `actionContext` there is now a **compile error** rather than a runtime `undefined`. `getInfo` was switched to `publicProcedureWithTelemetry` so its cast is truthful. _Reason:_ the old required-field-plus-cast was unsound; `tsc` now enforces the invariant everywhere.
- **F4 — `34dc09ee`, `5f07c5f5`, `4c7cf5c9`.** Both skill guides re-pointed to the current surface (`telemetryMiddlewareBody` + DocumentDB `TelemetryRunner`, `ctx.actionContext.telemetry`). _Reason:_ the guides taught APIs this PR reshaped; contributors and AI agents follow them verbatim.
- **F5 — `797f007b`.** Comment-only refresh of the three `trpcToTelemetry` references in `queryInsightsEventsRouter.ts` to the `telemetryMiddlewareBody` / runner model. _Reason:_ the named helper no longer exists after the reshape.
- **C1 — resolved, no code change.** Thread closed with the `@trpc/server@11.10.0` shallow-merge reference (`ctx: nextOpts?.ctx ? { ...opts.ctx, ...nextOpts.ctx } : opts.ctx`). _Reason:_ the reported context loss does not occur; enrichment is layered over the base context.

### Discrepancies worth flagging (things easy to miss)

1. **F4 pivoted mid-fix — the GitHub F4 comment is now stale.** The F4 timeline comment states the skill guides "removed the `type WithTelemetry` import and every `ctx as WithTelemetry<RouterContext>` cast" in favor of `ctx as RouterContext`. That was true at commit `34dc09ee`, but the later F3 fix (`efe086ee`) **re-introduced `WithTelemetry`** as the enrichment type, and commits `5f07c5f5`/`4c7cf5c9` then re-added `WithTelemetry<RouterContext>` casts to the guides. The **final** guides (and this doc's F4 body above) therefore teach `WithTelemetry<RouterContext>` narrowing — the opposite of what the F4 comment and the original F4 recommendation describe. Only the doc/PR-comment narrative is inconsistent; the shipped code and guides are self-consistent. Worth a one-line correction on the PR so a future reader is not misled.
2. **C1 regression test never landed.** The merge-preservation behavior is only exercised indirectly. `telemetry.test.ts` has no test that starts from a non-empty base context and asserts base fields (`signal`, `sessionId`, `clusterId`) survive the enrichment. Low cost, and it locks the finding against a future tRPC bump — recommend adding it rather than leaving it as an open offer.
3. **The original F1 "narrow the consumed range" alternative was not taken (correctly).** The author chose the minor bump over pinning consumers to `~0.9.1`. That is the more robust of the two options; no action needed, just noting the alternative was consciously dropped.
