...
### 4.4 What we could land today, with data the codebase already has

Almost everything in §4.3 needs a new data source. A health summary built only from today's four
checks must therefore **never** read as a verdict. The draft's green "No issues" row does exactly
that, and is misleading: it should state which checks ran, not that the cluster is fine.

The items below need no new SDK, no new permission and, except the last, no new server command.
Each is data the extension already reads somewhere and the dashboard drops or ignores.

| #   | What                                            | Already in the codebase                                                                                                                                                                                                                                                           | What it would give the page                                                                                                                                                                                                                                                                                                             | Cost                                                                                                                                        |
| --- | ----------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------- |
| T1  | **Why the cluster is unreachable**              | The ping sample's `errors[]`; `common.explainOperationFailure` → `ConnectionDiagnosticsService` with the Quick Start (Docker), Kubernetes (port-forward) and Atlas providers. Collection View, Document View and Query Insights already call it; the dashboard does not           | "Disconnected" becomes a finding with a cause and a next step: "Docker is not running" or "DocumentDB Local is stopped", "no active port-forward tunnel", or, for Atlas, "check the IP access list and whether the cluster is paused"                                                                                                   | One query when the connection state flips to disconnected                                                                                   |
...
- **Draft.** [`clusterHealthModel.ts`](../../../../../../src/webviews/documentdb/clusterDashboard/components/clusterHealthModel.ts)
  ...
  cluster is a replica_ (ARM `replicaRole`). Findings are sorted by severity. The subtitle reads
  "N checks ran; none reported an issue", but the empty-state row still shows a green "No issues",
  which overstates four checks. A shipped version must not render a healthy state until the
  sources in §4.4 and §6 exist.
