...
export class ShellScriptRunner extends vscode.Disposable {
    ...
    private static async _determineShellPathOrCmd(
        ...
        if (!shellPathSetting) {
            ...
            if (await cpUtils.commandSucceeds('mongosh', '--version')) {
                ...
                const noMongoError: string = l10n.t(
                    'This functionality requires the Mongo DB shell, but we could not find it in the path or using the documentDB.mongoShell.path setting.',
                );
