...
import { type UsageImpact } from './surveyTypes';

// Survey Configuration
const SurveyConfig = {
    urls: {
        DOCUMENTDB: 'https://aka.ms/DocumentDBSurvey',
    },
    settings: {
        ...
        MIN_SESSIONS_BEFORE_PROMPT: 2, // Sessions required before eligible for prompting
...
/**
 * Opens the survey in the default browser
 * @param triggerAction Optional action that triggered the survey for telemetry
 */
export function openSurvey(triggerAction?: string): void {
    ...
    void callWithTelemetryAndErrorHandling('survey.open', (context: IActionContext) => {
        ...
        void env.openExternal(
            ...
            Uri.parse(SurveyConfig.urls.DOCUMENTDB),
        );
    });
}

/**
 * Increments the usage score
 * @param score The amount to increment the score by
 */
export function countUsageForSurvey(score: UsageImpact | number): void {
    if (getIsSurveyDisabledGlobally() || surveyState.wasPromptedInSession) {
        return;
    }
    surveyState.usageScore = Math.min(SurveyConfig.scoring.MAX_SCORE, surveyState.usageScore + Math.max(0, score));
}
...
async function surveyPromptIfCandidate(triggerAction?: string): Promise<void> {
    ...
    await callWithTelemetryAndErrorHandling('survey.prompt', async (context: IActionContext) => {
        ...
        const button = await window.showInformationMessage(
            l10n.t('Got a moment? Share your feedback on DocumentDB for VS Code!'),
            take,
            remind,
            never,
        );
