...
export async function pasteCollection(context: IActionContext, targetNode: CollectionItem): Promise<void> {
    ...
    // Check type of sourceNode or targetNode
    ...
    if (!(sourceNode instanceof CollectionItem) || !(targetNode instanceof CollectionItem)) {
        void vscode.window.showWarningMessage(vscode.l10n.t('Invalid source or target node type.'));
        return;
    }

    // Confirm the copy operation with the user
    const sourceInfo = l10n.t(
        'Source: Collection "{0}" from database "{1}"',
        sourceNode.collectionInfo.name,
        sourceNode.databaseInfo.name,
    );
    const targetInfo = l10n.t(
        'Target: Collection "{0}" from database "{1}"',
        ...
        await vscode.window.withProgress(
            {
                ...
                title: l10n.t('Initializing copy task...'),
                cancellable: true,
            },
            async (progress, token) => {
                progress.report({ increment: 0, message: l10n.t('Copying documents…') });
                // Handle cancellation
                ...
                let lastProgress = 0;
                while (
                    task.getStatus().state === TaskState.Running ||
                    task.getStatus().state === TaskState.Initializing
                ) {
                    ...
                    if (currentProgress > lastProgress) {
                        progress.report({
                            increment: ((currentProgress - lastProgress) / totalDocuments) * 100,
                ...
                if (finalStatus.state === TaskState.Completed) {
                    progress.report({
                        increment: 100 - (lastProgress / totalDocuments) * 100,
