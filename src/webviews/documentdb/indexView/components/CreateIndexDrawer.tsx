 ...
 * picks one of these. The drawer later collapses the list of per-row choices
 * into an index-level `CreateIndexType` for the backend payload.
 *
 * - `asc` / `desc` → single-field (or compound) B-tree index, with the
 *   direction recorded for that specific key.
 * - `text` / `geospatial` / `ttl` → applies to the field that owns the row;
 *   the index inherits that type at submit time.
 */
 ...
 * can be empty mid-edit). `typeChoice` is undefined until the user picks one,
 * so we can disable submission until every populated row has a type.
 */
 ...
 * DocumentDB itself supports — a "text" field forces the whole index to be a
 * text index, geo wins over ttl, etc. When every row is asc/desc we fall back
 * to a (possibly compound) single-field index.
 */
 ...
 * end (right) of the panel. The form state is intentionally preserved when the
 * drawer is dismissed — users who set up a complex compound index do not lose
 * their work on an accidental close — and is only cleared via "Reset form" or
...
export const CreateIndexDrawer = ({
    ...
    /**
     * A row is "complete" only when both the field name and the type have been
     * chosen. We only ship complete rows to the backend so users can leave
     * blank scaffolding rows in the form without breaking submit.
     */
            ...
            <DrawerBody className="createIndexDrawerBody">
                <div className="createIndexForm">
                    ...
                    <DrawerSection
                        ...
                        hint={l10n.t(
                            'Select the field(s) to index and a sort order or special type for each. Add more fields to build a compound index.',
                        )}
                    >
                            ...
                            {fields.map((draft) => (
                                <div key={draft.id} className="fieldRow">
                                    ...
                                    <Dropdown
                                        ...
                                        value={
                                            draft.typeChoice
                                                ? (typeLabels.find((t) => t.value === draft.typeChoice)?.label ?? '')
                                                : ''
                                        }
                                        onOptionSelect={(_, data) =>
                                            updateField(draft.id, {
                                                typeChoice:
                                                    (data.optionValue as IndexTypeChoice | undefined) ?? undefined,
                                            })
                                        }
                    ...
                    {showSingleFieldOptions && (
                        <DrawerSection
                            ...
                            hint={l10n.t(
                                'Unique rejects duplicate values; Sparse only indexes documents that contain the field.',
                            )}
                        >
                    ...
                    {showTtlOptions && (
                        <DrawerSection
                            title={l10n.t('Expiration')}
                            hint={l10n.t('Documents are automatically removed once they reach the specified age.')}
                        >
                            <div className="typeOptions">
                                <Field
