...
| `trpc.ts`            | host                      | The extension's one tRPC instance (`initWebviewTrpc`). Exports `publicProcedure`, `router`, `createCallerFactory`, `publicProcedureWithTelemetry`, and the `RpcEnrichment` shape, and owns the DocumentDB `TelemetryRunner` that contributes the `IActionContext` to `ctx.actionContext` and reports RPC telemetry to Application Insights. Kept as a leaf module so per-view routers can import it without a circular dependency on `appRouter.ts`. |
