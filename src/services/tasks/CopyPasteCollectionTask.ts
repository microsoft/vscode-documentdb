...
export class CopyPasteCollectionTask extends Task {
    ...
    protected async onInitialize(signal: AbortSignal): Promise<void> {
        ...
        this.updateStatus(
            ...
            vscode.l10n.t('Counting documents in source collection...'),
            0,
        );
        ...
        try {
            ...
            throw new Error(
                vscode.l10n.t('Failed to count documents in source collection: {0}', 
        ...
        this.updateStatus(
            this.getStatus().state,
            vscode.l10n.t('Ensuring target collection exists...'),
            0,
        );
        ...
        try {
            ...
            throw new Error(
                vscode.l10n.t('Failed to ensure target collection exists: {0}', 
                    error instanceof Error ? error.message : String(error)
                ),
            );
        }
    ...
    protected async doWork(signal: AbortSignal): Promise<void> {
        ...
        try {
            ...
            if (this.config.onConflict === ConflictResolutionStrategy.Abort) {
                throw new Error(
                    vscode.l10n.t('Copy operation failed: {0}', 
                        error instanceof Error ? error.message : String(error)
                    ),
                );
            }
    ...
    private async flushBuffer(buffer: DocumentDetails[]): Promise<void> {
        ...
        if (result.errors.length > 0) {
            ...
            if (this.config.onConflict === ConflictResolutionStrategy.Abort) {
                ...
                throw new Error(
                    vscode.l10n.t('Write operation failed: {0}', firstError.error.message),
                );
            }
            // Future: Handle other conflict resolution strategies
        }
