...
import { BufferErrorCode, createMongoDbBuffer } from '../../utils/documentBuffer';
...
export class CopyPasteCollectionTask implements Task {
    ...
    private copiedDocumentCount: number = 0;
    ...
    constructor(
        ...
        void reader
            .countDocuments(config.source.connectionId, config.source.databaseName, config.source.collectionName)
            .then((count) => {
    ...
    private async executeTask(): Promise<void> {
        try {
            ...
            this.updateStatus({
                ...
                progress: 100,
                message: `Successfully copied ${this.copiedDocumentCount} documents`,
            });
        } catch (error) {
     ...
     */
    private async streamDocuments(): Promise<void> {
        ...
        for await (const document of documents) {
            ...
            if (insertResult.success) {
                // Successfully inserted into buffer, continue to next document
                continue;
            }

            // Handle insert failures
            if (insertResult.errorCode === BufferErrorCode.DocumentTooLarge) {
                // Document is too large for buffer, handle immediately
                await this.writeDocuments([document]);
                continue;
            } else if (insertResult.errorCode === BufferErrorCode.BufferFull) {
                // Buffer is full, flush first
                if (this.documentBuffer.getStats().documentCount > 0) {
                    await this.flushBuffer();
                }
                // Insert again after flush
                // We checked for DocumentTooLarge above, so we can safely retry
                const retryInsertResult = this.documentBuffer.insert(document);
                if (!retryInsertResult.success) {
                    // If still fails, log the error and continue
                    ext.outputChannel.appendLog(
                        l10n.t(
                            'Failed to insert document with id {0} into buffer: {1}',
                            document.id as string,
                            String(retryInsertResult.errorCode),
                        ),
                    );
                }
                continue;
    ...
    private async writeDocuments(documents: DocumentDetails[]): Promise<void> {
        try {
            ...
            this.copiedDocumentCount += result.insertedCount;
            this.updateProgress(this.copiedDocumentCount);
    ...
    private updateProgress(current: number): void {
        const progress = Math.min(Math.round((current / this.totalDocuments) * 90) + 10, 100); // Reserve 10% for setup
        this.updateStatus({
            state: TaskState.Running,
            progress,
            message: `Copied ${current} of ${this.totalDocuments} documents`,
        });
    }
