...
import { AuthMethodId } from '../../src/documentdb/auth/AuthMethod';
import {
    type CollectionStats,
    type IndexSpecification,
    type IndexStats,
} from '../../src/documentdb/llmEnhancedFeatureApis';
...
async function getOrCreateClusterId(config: TestConfig): Promise<string | undefined> {
    ...
    throw new Error(
        'Either clusterId with registered credentials or connectionString is required in test configuration',
    );
}
 ...
 * @param isCSVMode Whether to run in CSV mode
...
export async function executeTestCase(
    ...
    try {
        if (!isCSVMode) {
            ...
            progress?.(`Using pre-loaded execution data...`);

            // Build query context with pre-loaded data
            queryContext = {
                databaseName: config.databaseName,
                collectionName: testCase.collectionName,
                commandType: CommandType.Find, // Default for now
                executionPlan: testCase.executionPlan,
                collectionStats: testCase.collectionStats as CollectionStats | undefined,
                indexStats: testCase.indexStats as IndexStats[] | undefined,
                preferredModel: config.preferredModel,
                fallbackModels: config.fallbackModels,
            };
        } else {
            if (!testCase.query) {
                throw new Error('Query is required for CSV-based test mode');
            }
            ...
            clusterId = await getOrCreateClusterId(config);
            if (!clusterId) {
        ...
        if (!skipPerformance && result.suggestions) {
            try {
                ...
                progress?.(
                    `Performance measurement failed: ${perfError instanceof Error ? perfError.message : String(perfError)}`,
                );
...
async function measureQueryPerformance(
    ...
    if (config.shouldWarmup !== false) {
        try {
            // execute list for now update in later version
    ...
    try {
        ...
        let explainResult;
        let findParams: ReturnType<typeof parseFindQueryParameters> | undefined;

        if (queryContext.commandType === CommandType.Find) {
            // Validate query exists
            if (!queryContext.query) {
                throw new Error('Query is required for Find command type');
            }
        ...
        for (const key in findParams) {
            const value = findParams[key as keyof typeof findParams];
            if (key === 'skip' || key === 'limit') {
                findParamsCopy[key] = value as number;
            } else {
                if (typeof value === 'object' && value !== null) {
                    findParamsCopy[key] = JSON.stringify(value);
                }
            }
        }
        ...
        if (queryContext.commandType === CommandType.Find) {
            ...
            const commandParts: string[] = [
                `db.${queryContext.databaseName}.${queryContext.collectionName}.find(${filterStr})`,
            ];
            ...
            progress?.(
                `Executing: db.${queryContext.databaseName}.${queryContext.collectionName}.countDocuments({...})`,
            );
        }
        ...
        for (let i = 0; i < tries - 1; i += 1) {
            if (queryContext.commandType === CommandType.Find) {
                ...
                await client.runFindQuery(queryContext.databaseName, queryContext.collectionName, findParamsCopy);
            } else if (queryContext.commandType === CommandType.Aggregate) {
                startTime = performance.now();
            } else if (queryContext.commandType === CommandType.Count) {
                startTime = performance.now();
 ...
 */
function detectIndexAction(mongoShellCommand: string): 'create' | 'drop' | 'modify' | 'none' {
    ...
    if (command.includes('dropindex')) {
        ...
        return 'create';
    } else if (command.includes('updateindex')) {
        return 'modify';
    } else {
        return 'none';
    }
...
async function dropIndexFromSuggestion(
    ...
    try {
        ...
        const indexName = extractIndexNameFromDropCommand(suggestions);
...
async function restoreOriginalIndexState(
    ...
    databaseName: string,
    collectionName: string,
    actionType: 'create' | 'drop' | 'modify' | 'none',
    ...
    try {
                ...
                try {
                    ...
                    if (matchedIndex && matchedIndex.key) {
                        ...
                        if (indexNameToDrop !== '_id_') {
                            ...
                            try {
                                const recreateResult = await client.createIndex(
                                    databaseName,
                                    collectionName,
                                    indexSpecToRestore,
                                );
                                if (recreateResult.ok === 1) {
                                    ...
                                    progress?.(
                                        `Warning: Failed to recreate index ${indexNameToDrop}: ${recreateResult.note || 'Unknown error'}`,
                                    );
                                }
                            } catch (recreateError) {
                                progress?.(
                                    `Warning: Error recreating index ${indexNameToDrop}: ${recreateError instanceof Error ? recreateError.message : String(recreateError)}`,
                                );
                            }
                        }
                    } else {
                    ...
                    progress?.(
                        `Warning: Failed to parse indexStats for restoration: ${parseError instanceof Error ? parseError.message : String(parseError)}`,
                    );
                }
            }
        } else if (actionType === 'modify') {
        ...
        progress?.(
            `Warning: Failed to restore original state: ${error instanceof Error ? error.message : String(error)}`,
        );
 ...
 * @param config Test configuration
 * @param isCSVMode Flag indicating if the test is running in CSV mode
 * @param progress Optional progress callback
 */
export async function warmupConnection(
    ...
    if (!isCSVMode || config.shouldWarmup === false) {
        progress?.('Skipping connection warmup for directory-based mode');
        return;
    }
