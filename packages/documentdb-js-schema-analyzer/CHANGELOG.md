...
`getKnownFields()` and `getSchema()` output changes for any document containing BSON wrapper values: correct field paths and types replace the wrapper internals, and `MinKey`/`MaxKey` fields reappear. Invalidate any persisted or cached schema derived from an earlier version.
