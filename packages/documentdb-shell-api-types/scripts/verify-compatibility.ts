...
function parseCommandsTable(markdown: string): CommandStatus[] {
    ...
    const section = nextSection === -1 ? markdown.substring(dbCmdStart) : markdown.substring(dbCmdStart, nextSection);
...
function verify(
    ...
    const supportedSet = new Set(scrapedCommands.filter((c) => c.supported).map((c) => c.command.toLowerCase()));

    const allScrapedSet = new Set(scrapedCommands.map((c) => c.command.toLowerCase()));
    ...
    const unsupported = requiredLower.filter((c) => allScrapedSet.has(c) && !supportedSet.has(c));
    ...
    const extraSupported = [...supportedSet].filter((c) => !referencedLower.has(c));
...
function generateReport(
    ...
    if (result.unsupported.length > 0) {
        ...
        for (const cmd of result.unsupported) {
            const methods = SHELL_API_METHODS.filter((m) => m.serverCommands.some((sc) => sc.toLowerCase() === cmd));
    ...
    // Unified command table — shows every scraped command with support + implementation status
    const referencedCommandsLower = new Set(requiredCommands.map((c) => c.toLowerCase()));

    lines.push('## Server Command Coverage');
    lines.push('');
    lines.push('Every command from the official compatibility page, showing whether it is');
    lines.push('supported by DocumentDB and whether our shell API type definitions include');
    lines.push('methods that use it.');
    lines.push('');
    lines.push('| Command | Category | Supported | In API Types | Shell Methods |');
    lines.push('| --- | --- | --- | --- | --- |');
    for (const cmd of [...scrapedCommands].sort((a, b) => a.command.localeCompare(b.command))) {
        const supportedMark = cmd.supported ? '✅ Yes' : '❌ No';
        const isReferenced = referencedCommandsLower.has(cmd.command.toLowerCase());
        const implementedMark = isReferenced ? '✅ Yes' : '';
        const methods = isReferenced
            ? SHELL_API_METHODS.filter((m) =>
                  m.serverCommands.some((sc) => sc.toLowerCase() === cmd.command.toLowerCase()),
              )
                  .map((m) => `${m.target}.${m.name}`)
                  .join(', ')
            : '';
        lines.push(`| ${cmd.command} | ${cmd.category} | ${supportedMark} | ${implementedMark} | ${methods} |`);
    }
    lines.push('');

    lines.push('## Shell-Only Methods (no server command)');
    lines.push('');
    lines.push('These methods run entirely on the client side and do not invoke a server command.');
    lines.push('');
    lines.push('| Target | Method | Description |');
    lines.push('| --- | --- | --- |');
    for (const m of SHELL_API_METHODS.filter((m) => m.shellOnly)) {
        lines.push(`| ${m.target} | ${m.name} | ${m.description} |`);
    }
    lines.push('');
...
async function main(): Promise<void> {
    ...
    if (result.missing.length > 0) {
        console.log(
            '[SHELL-API-MISSING-COMMAND] the following commands are referenced but not found in the docs table:',
        );
        for (const cmd of result.missing) {
    ...
    if (result.unsupported.length === 0 && result.missing.length === 0) {
        ...
        console.log(
            `   (${requiredCommands.length} server commands checked, ${SHELL_API_METHODS.length} shell methods total)`,
        );
    }
    if (result.extraSupported.length > 0) {
        console.log(
