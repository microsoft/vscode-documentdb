...
/**
 * Parses the scraped index-reference.md dump into its "Index Types" and
 * "Index Properties" tables (Name | Description | Supported).
 */
function parseIndexReference(content: string): { types: ParsedIndexEntry[]; properties: ParsedIndexEntry[] } {
    const sections = new Map<string, ParsedIndexEntry[]>();
    let current = '';
    ...
    for (const raw of content.split('\n')) {
        ...
        if (!separatorSeen) continue; // header row

        sections.get(current)!.push({
            name: cells[0],
            description: cells[1],
            supported: /^yes$/i.test(cells[2]),
        });
    }

    return {
        types: sections.get('index types') ?? [],
        properties: sections.get('index properties') ?? [],
    };
}

/**
 * Generates the src/indexReference.ts file content from the parsed index dump.
 */
function generateIndexReferenceContent(index: { types: ParsedIndexEntry[]; properties: ParsedIndexEntry[] }): string {
    const copyright = `/*---------------------------------------------------------------------------------------------
 *  Copyright (c) Microsoft Corporation. All rights reserved.
 *  Licensed under the MIT License. See License.txt in the project root for license information.
 *--------------------------------------------------------------------------------------------*/
...
`;
    ...
    const emitArray = (constName: string, entries: ParsedIndexEntry[]): string => {
        const rows = entries
            .map(
                (e) =>
                    `    { name: '${escapeString(e.name)}', description: '${escapeString(e.description)}', supported: ${e.supported} },`,
            )
            .join('\n');
...
function main(): void {
    ...
    if (fs.existsSync(indexDumpPath)) {
        ...
        const index = parseIndexReference(fs.readFileSync(indexDumpPath, 'utf-8'));
        console.log(`  ${index.types.length} index types, ${index.properties.length} index properties`);
        const indexFilePath = path.join(srcDir, 'indexReference.ts');
        fs.writeFileSync(indexFilePath, generateIndexReferenceContent(index), 'utf-8');
        generatedIndexFiles.push(indexFilePath);
    } else {
        console.log('\nℹ️  No index-reference.md dump found, skipping indexReference.ts.');
    }

    // Format generated files with Prettier
    const generatedFiles = [...[...fileGroups.keys()].map((f) => path.join(srcDir, `${f}.ts`)), ...generatedIndexFiles];
    ...
    for (const f of generatedIndexFiles) {
        console.log(`  src/${path.basename(f)}`);
    }
}
